...
class Pagination:
    ...
    def string_pager(self, base_url="/index/"):
        ...
        if self.current_page == 1:
            prev = '<li><a href="javascript:void(0);" class="uppage">上一页</a></li>'
        else:
            prev = '<li><a href="%s?page=%s" class="uppage">上一页</a></li>' % (base_url, self.current_page - 1,)
        list_page.append(prev)

        for p in range(s, t):  # 1-11
            if p == self.current_page:
                # temp = '<li><a class="active pagenumber" href="%s?page=%s">%s</a></li>' % (base_url,p, p)
                temp = '<li><span class="current_page_span">%s</span></li>' % (p,)
            else:
                temp = '<li><a href="%s?page=%s" class="pagenumber">%s</a></li>' % (base_url,p, p)
            list_page.append(temp)
        if self.current_page == self.all_pager:
            nex = '<li><a href="javascript:void(0);" class="downpage">下一页</a></li>'
        else:
            nex = '<li><a href="%s?page=%s" class="downpage">下一页</a></li>' % (base_url, self.current_page + 1,)
